...
export function SceneView({ sceneId, progress, className }: SceneProps) {
  return (
    <svg viewBox="0 0 320 200" className={className} preserveAspectRatio="xMidYMid slice" aria-hidden="true">
  ...
  return (
    <g>
      ...
      {[40, 92, 150, 210, 268].map((x, i) => (
        <g key={x} opacity={fade(p, 0.6 + i * 0.06)} transform={`translate(${x},${168 - (i % 2) * 6})`}>
          <line x1="0" y1="0" x2="0" y2="16" stroke="#3f6f2e" strokeWidth="2" />
          <circle cx="0" cy="-2" r="5" fill={['#f06a8a', '#f6b73c', '#c98bd6', '#ef7d63', '#f4d04a'][i]} />
...
function Windowsill({ p }: { p: number }) {
  return (
    <g>
      ...
      <rect x="36" y="20" width="248" height="150" rx="8" fill="#9cc2dc" opacity={fade(p, 0.25) * 0.5} />
      {[...Array(7)].map((_, i) => (
        <line
          key={i}
          x1={60 + i * 30}
  ...
  return (
    <g>
      ...
      {[
        ...
        <circle key={i} cx={x} cy={y} r={i % 3 === 0 ? 1.8 : 1.1} fill="#fdf6d8" opacity={fade(p, 0.25 + i * 0.05)} />
      ))}
